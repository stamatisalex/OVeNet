...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        if (self.offset_branch<= 4):
            ...
            self.stage4_2, pre_stage_channels = self._make_stage(
                self.stage4_2_cfg, num_channels, multi_scale_output=True)
        ...
        self.aux_head_offset = nn.Sequential(
            nn.Conv2d(last_inp_channels, last_inp_channels,
                      kernel_size=1, stride=1, padding=0),
            BatchNorm2d(last_inp_channels),
            nn.ReLU(inplace=relu_inplace),
            nn.Conv2d(last_inp_channels, 3,
                      kernel_size=1, stride=1, padding=0, bias=True)
        )
    ...
    def forward(self, x):
        ...
        if (self.offset_branch <= 3):
            ...
            for i in range(self.stage3_cfg['NUM_BRANCHES']):
                if self.transition2_2[i] is not None:
                    if i < self.stage2_cfg['NUM_BRANCHES']:
                        x2_list.append(self.transition2_2[i](y_list[i]))
                    else:
        ...
        if (self.offset_branch <= 4):
            ...
            for i in range(self.stage4_cfg['NUM_BRANCHES']):
                if self.transition3_2[i] is not None:
                    if i < self.stage3_cfg['NUM_BRANCHES']:
                        x2_list.append(self.transition3_2[i](y_list[i]))
                    else:
        ...
        x2_1 = F.interpolate(x_2[1], size=(x2_h, x2_w),
                        mode='bilinear', align_corners=ALIGN_CORNERS)
        x2_2 = F.interpolate(x_2[2], size=(x2_h, x2_w),
                        mode='bilinear', align_corners=ALIGN_CORNERS)
        x2_3 = F.interpolate(x_2[3], size=(x2_h, x2_w),
                        mode='bilinear', align_corners=ALIGN_CORNERS)

        feats = torch.cat([x[0], x1, x2, x3], 1)
        feats_offset = torch.cat([x_2[0], x2_1, x2_2, x2_3], 1)

        # out_aux_seg = []

        # ocr
        s_i_i = self.aux_head(feats)  #batch x 19 x h x w
        # compute contrast feature
        feats = self.conv3x3_ocr(feats)

        context = self.ocr_gather_head(feats, s_i_i)
        feats = self.ocr_distri_head(feats, context)

        s_i_o = self.cls_head(feats) # batch x 19 x h x w

        batch_size,C,H,W = s_i_o.size()
        if ( not self.logits):
            s_i_o = F.softmax(s_i_o, dim=1)
        ...
        o_f =self.aux_head_offset(feats_offset) # batch x 3 x h x w
        ...
        s_s_i = F.grid_sample(s_i_i, ocoords, padding_mode="border")
        s_s_o = F.grid_sample(s_i_o, ocoords, padding_mode="border")
        ...
        s_f_i = (1-confidence_map) * s_i_i + confidence_map * s_s_i
        s_f_o = (1-confidence_map) * s_i_o + confidence_map * s_s_o


        offset = offset.permute(0, 3, 1, 2)
        s_i_list = []
        s_i_list.append(s_i_i)
        s_i_list.append(s_i_o)

        s_s_list = []
        s_s_list.append(s_s_i)
        s_s_list.append(s_s_o)

        s_f_list = []
        s_f_list.append(s_f_i)
        s_f_list.append(s_f_o)


        return s_i_list, offset, s_s_list, s_f_list, confidence_map
    ...
    def init_weights(self, pretrained='',pretrained_offset = ''):
        ...
        if os.path.isfile(pretrained):
            ...
            pretrained_dict_offset = torch.load(pretrained_offset,map_location={'cuda:1': 'cpu'})
            ...
            for k, _ in pretrained_dict_offset.items():
                logger.info(
                    '=> loading {} pretrained model {}'.format(k, pretrained_offset))
            ...
            if (cfg.MODEL.EXTRA.OFFSET_BRANCH <= 3):
                ...
                if (cfg.MODEL.EXTRA.OFFSET_BRANCH <= 2):
                    ...
                    for param in model.transition1_2.parameters():
                        param.requires_grad = True
        for param in model.aux_head_offset.parameters():
            param.requires_grad = True
