...
class BaseDataset(data.Dataset):
    ...
    def inference(self, model, image, flip=False, debug=False):
        ...
        if config.MODEL.NUM_OUTPUTS > 1:
            ...
            s_s = s_s[config.TEST.OUTPUT_INDEX]
            scores = scores[config.TEST.OUTPUT_INDEX]
        ...
        if flip:
            ...
            if config.MODEL.NUM_OUTPUTS > 1:
                ...
                s_s_flip_output = s_s_flip_output[config.TEST.OUTPUT_INDEX]
                scores_flip_output = scores_flip_output[config.TEST.OUTPUT_INDEX]
